...
public class HumanPyramid {
   ...
   */
  private double helper(List<List<Double>> pyramid, int row, int col) {
    if (row == 0 && col == 0) {
      return pyramid.get(0).get(0);
    }
    double addOn = 0.0;
    if (col == 0) {
      addOn = 0.5 * helper(pyramid, row - 1, col);
    } else if (row == col) {
      addOn = 0.5 * helper(pyramid, row - 1, col - 1);
    } else {
      addOn = 0.5 * helper(pyramid, row - 1, col)
          + 0.5 * helper(pyramid, row - 1, col - 1);
    }
    return pyramid.get(row).get(col) + addOn;
  }

  public float totalWeightSupported(List<List<Double>> pyramid, int row, int col) {
    // return helper(pyramid,row,col);
    return (float) helper(pyramid, row, col);
  }
